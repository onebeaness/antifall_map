...
## 2026-08 수집 결과 (생활 보행로 way 31,695개 기준)

    다리      1,091  ( 3.4%)   ← 표고 보정에 씀
    터널        515  ( 1.6%)   ← 표고 보정에 씀
    노면재질  4,136  (13.1%)   참고용
    폭          509  ( 1.6%)   ※ 아래 참고
    incline     126  ( 0.4%)   값이 'up'/'down'/'0%' 뒤죽박죽 — 안 씀
    계단      3,125개 way (우리 자료엔 0개, 별도 지표 검토 중)

**폭 태그는 팀 산출물의 출처가 아니다.** OSM에 폭이 붙은 way는 509개인데
(way당 평균 4지점 → 최대 2,030지점) 팀 산출물에는 33,767지점에 폭이 있다.
16배 차이라 같은 출처일 수 없다. 협소 판정의 근거를 팀에 확인해야 한다.

수집 결과는 data/osm_walkway_tags.json으로 저장돼 있고 build_risk_geojson.py가
자동으로 읽는다. 갱신하려면 이 스크립트를 다시 돌리면 된다.
