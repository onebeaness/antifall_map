...
export function TrendLine({ labels, values, unit = "건", ariaLabel = "추이 차트" }: {
  labels: string[];
  values: number[];
  unit?: string;
  ariaLabel?: string;
  ...
  // 눈금은 어중간한 값 대신 1·2·5 배수의 "나이스" 간격으로
  const rawStep = maxV / 4;
  const mag = 10 ** Math.floor(Math.log10(rawStep));
  const step = Math.ceil(rawStep / mag) * mag;
  const yTicks = [1, 2, 3, 4].map((i) => i * step).filter((v) => v <= maxV);
    ...
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: "auto", fontFamily: "inherit" }}
      ...
      {yTicks.map((v, i) => (
        <g key={i}>
          ...
          <text x={PAD.left - 6} y={y(v) + 4} textAnchor="end" fontSize={11} fill="var(--ink-muted)">{v.toLocaleString()}</text>
        </g>
      ))}
